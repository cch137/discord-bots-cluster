...
async function googleSearchSummary(query: string) {
  return (await axios.get(`https://${API_ORIGIN}/google-search-summary?showUrl=true&query=${query}`)).data as string
}

async function ddgSearchSummary(query: string) {
  return (await axios.get(`https://${API_ORIGIN}/ddg-search-summary?showUrl=true&query=${query}`)).data as string
}
...
async function disconnect () {
  ...
  console.log(`CH4 disconneted in ${Date.now() - t0} ms`)
}
...
async function connect () {
  ...
  await client.login(process.env.CH4_TOKEN)
  ...
  (async () => {
    ...
    async function logStatus() {
      const result = (await axios.get('https://ch4.us.to/api/status')).data as { models: [string,number][], totalMessages: number, totalRegisteredUsers: number, dataSize: number }
      ...
      targetMessage.edit({
        ...
        embeds: [
          new EmbedBuilder().setFields(...[
            ...
            { name: 'Database:', value: [`Total Messages: ${result.totalMessages}`, `Registered Users: ${result.totalRegisteredUsers}`, `Data Size: ${formatBytes(result.dataSize)}`].join('\n') },
          ].filter(f => f.name))
        ]
      })
    }
  ...
  client.on('messageCreate', async (message) => {
    ...
    if (content.at(0) === '_') {
      ...
      switch (command) {
        ...
        case '_google':
          if (args.length) {
            replyWithCodeBlocks(message, await googleSearchSummary(args.join(' ')))
          }
          break
        case '_ddg':
        case '_duckduckgo':
          if (args.length) {
            replyWithCodeBlocks(message, await ddgSearchSummary(args.join(' ')))
          }
          break
        case '_rand':
        case '_random':
  ...
  console.log(`CH4 conneted in ${Date.now() - t0} ms`)
  return
}
